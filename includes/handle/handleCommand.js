    ...
    if (commandName.startsWith(PREFIX)) {
      if (!command) {
        ...
        if (checker.bestMatch.rating >= 0.5) {
          ...
          return api.shareContact("𝖳𝗁𝖾 𝖼𝗈𝗆𝗆𝖺𝗇𝖽 𝗒𝗈𝗎 𝗍𝗒𝗉𝖾 𝗂𝗌 𝗇𝗈𝗍 𝖿𝗈𝗎𝗇𝖽", event.senderID, event.threadID);
        }
      }
    }
  ...
  command = [...commands.values()].find(cmd => (
   ...
   cmd.config.usePrefix === false
    ...
    if (command && command.config && command.config.hasPermssion && command.config.hasPermssion > permssion) { 
      return api.sendMessage(`📫 | You have no permission this command "${command.config.name}" i think you're not administrator group chat or Adminbot`, event.threadID, event.messageID); 
    }
